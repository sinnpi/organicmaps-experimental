...
public class SearchFragment extends Fragment implements SearchListener, CategoriesAdapter.CategoriesUiListener
{
  ...
  private final Observer<Boolean> mSearchEnabledObserver = new Observer<>() {
    public void onChanged(Boolean enabled)
    {
      ...
      if (!enabled)
      {
        ...
        // The sheet is only translated off-screen and the view is never recreated.
        resetPickerActions();
  ...
  // Location fixes and mode changes cannot end a pick, so they can skip the refresh entirely while
  // none is armed. Callers that can end one (updateFrames()) must stay unguarded to run the reset.
  ...
  private void updatePickerRows()
  {
    ...
    // A pick stays armed while the map chooser is up, and the location listener keeps firing on the
    ...
    if (picking)
      UiUtils.showIf(hasUsableMyPosition() && controller.canPickMyPosition(), mYourLocation);
  ...
  private boolean hasUsableMyPosition()
  {
    ...
    final Integer mode = mMapButtonsViewModel.getMyPositionMode().getValue();
    if (mode != null && mode == LocationState.NOT_FOLLOW_NO_POSITION)
      return false;
  ...
  private class ToolbarController extends SearchToolbarController
  {
    ...
    protected void onTextChanged(String query)
    {
      ...
      // updateFrames() only runs once the debounced search fires, too late to hide the shortcuts.
      // Only the first character matters: the rows are gone by the second.
